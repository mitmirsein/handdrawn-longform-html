...
Validate the outline with `scripts/validate_outline.py`. The HTML renderer applies `mix-blend-mode: multiply` to opaque reviewed illustrations; when actual alpha is required, use `python3 scripts/make_transparent.py output/<slug>/deck.json --in-place` or pass `--make-transparent` to `build_deck.py`. That command processes only slide-referenced scene images and never the anchor or stale screenshots. Assemble the reviewed local assets with `scripts/build_deck.py`, which renders static HTML first and then uses the system Chrome/Playwright adapter for PDF. The browser preflight must report no art/text geometry overlaps; fix the shared renderer or reviewed deck layout instead of editing a generated HTML file by hand. Run `scripts/audit_project.py --require-output` before sharing and use `scripts/package_share.py` to create a self-contained bundle. The legacy `scripts/render_pptx.py` remains an optional target. If a renderer is unavailable, leave a valid `deck.json` and prompt/reference manifest; do not claim an exported deck.
